...
SCHEMA_PATH = (
    ...
    / "schemas"
    / "wazuh_windows_security_auth_hit_projection.schema.json"
)

ADAPTER_NAME = "wazuh_windows_security_auth_hit_adapter"
ADAPTER_VERSION = "1.0"
...
WAZUH_OMITTABLE_SENTINEL_EVENT_DATA_FIELDS = {
    "subjectUserName",
    "subjectDomainName",
    "workstationName",
    "ipAddress",
...
class WazuhWindowsSecurityAuthAdaptError(ValueError):
    """Raised when a Wazuh authentication hit cannot be adapted safely."""
...
def _validate_projection(projection: Mapping[str, object]) -> None:
    ...
    if error is not None:
        ...
        raise WazuhWindowsSecurityAuthAdaptError(
            f"Wazuh Windows Security authentication projection validation failed at {path}"
        ) from None
...
def _validate_query_window(projection: Mapping[str, object]) -> None:
    ...
    if start >= end:
        raise WazuhWindowsSecurityAuthAdaptError(
            "Wazuh Windows Security authentication projection validation failed at "
            "retrieval.query_window"
        )
...
def _decimal_integer(value: object, *, path: str) -> int:
    if not isinstance(value, str) or not value.isascii() or not value.isdecimal():
        raise WazuhWindowsSecurityAuthAdaptError(
            f"Wazuh Windows Security authentication conversion failed at {path}"
        )
    return int(value, 10)


def adapt_wazuh_windows_security_auth_hit(
    projection: Mapping[str, object],
) -> dict[str, object]:
    """Convert one validated Wazuh hit projection to the reviewed source contract."""

    if not isinstance(projection, Mapping):
        raise WazuhWindowsSecurityAuthAdaptError(
            "Wazuh Windows Security authentication projection validation failed at projection"
        )
    ...
    for source_field in WAZUH_OMITTABLE_SENTINEL_EVENT_DATA_FIELDS:
        event_data.setdefault(EVENT_DATA_FIELD_MAPPING[source_field], "-")
    source_event = {
    ...
    return {"source_event": source_event, "retrieval_provenance": provenance}
