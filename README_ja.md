...
決定論的な検知、AIを利用した分析、エビデンスに基づく改善を
セキュリティ運用にどのように組み合わせられるか検証する個人ホームラボです。

攻撃シミュレーションからログ収集、検知、相関分析、トリアージ、調査、対応、
DFIR、検知ルール改善までを一つの流れとして実験します。本番環境への導入ではなく、
実装と検証を通じた学習・研究を目的としています。
